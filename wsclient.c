...
void *libwsclient_run_thread(void *ptr) {
	//debug("entering libwsclient_run_thread\n");
	...
	close(c->sockfd);
	free(c);
	//debug("leaving libwsclient_run_thread\n");
	return NULL;
}
...
void libwsclient_handle_control_frame(wsclient *c, wsclient_frame *ctl_frame) {
	...
	switch(ctl_frame->opcode) {
		...
		case 0x9:
			//ping frame
			//debug("ping received, payload length = %i\n", (int)ctl_frame->payload_len);
			//Send pong frame as acknowledgement.
			for(i=0;i<ctl_frame->payload_len;i++)
				*(ctl_frame->rawdata + ctl_frame->payload_offset + i) ^= (mask[i % 4] & 0xff); //mask payload
			*(ctl_frame->rawdata )    ^= 0x03; //turn ping opcode into a pong - 09 -> 0a
...
int libwsclient_helper_socket(wsclient *c, const char *path) {
	...
	pthread_create(&c->helper_thread, NULL, libwsclient_helper_socket_thread2, (void *)c);
}

void *libwsclient_helper_socket_thread2(void *ptr) {
	wsclient *c = ptr;
...
void *libwsclient_handshake_thread(void *ptr) {
	...
	snprintf(request_headers, 1024, "GET %s?encoding=text HTTP/1.1\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nHost: %s\r\nSec-WebSocket-Key: %s\r\nSec-WebSocket-Version: 13\r\n\r\n", path, request_host, websocket_key);
